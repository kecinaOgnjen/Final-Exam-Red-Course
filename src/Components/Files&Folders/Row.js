...
export default class Row extends Component {
    ...
    generateExpandElements(){
        ...
        if(this.state.expand && file && file.children){
    ...
    render() {
        ...
        return (
            <div style={{ borderBottom: '1px solid #eeeeee'}}>
                <div className="pointer item" onClick={this.expandFolder} onDoubleClick={() => this.props.changeDirectory(file)} style={{ flexDirection: 'row', height: 35, alignItems: 'center', display: 'flex', paddingLeft: 10}}>
                    <div style={{flex: 0.65}}>
                        <i id="file-upload" className={file.type == 'folder' ? "fas fa-folder" : "fas fa-file "}></i>&nbsp;
                        <span style={{paddingLeft: 10}}>
                            {file.name}
